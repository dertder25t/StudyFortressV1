...
// Version 1.3: Enhanced Error Handling and Logging
...
app.get('/api/all-data', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch app data from server." });
  }
});
...
app.post('/api/generate-ai-cards', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!apiKey) return res.status(400).json({ error: `API key for ${provider} not found in your profile.` });
        let cards;
        switch(provider) {
            ...
            default: return res.status(400).json({ error: 'Invalid provider specified.' });
        }
        res.json(cards);
    } catch (err) {
        console.error(`Error generating cards with ${provider}:`, err);
...
app.put('/api/cards/:id', authenticateToken, async (req, res) => {
    try {
        ...
        console.error("Error updating card SRS data:", err);
